...
#[instrument(name = "air: prove many 2 F/F", skip_all)]
...
#[instrument(name = "air: prove many 2 F/EF", skip_all)]
pub fn prove_many_air_2_f_and_ef<'a, EF: ExtensionField<PF<EF>>, A1: MyAir<EF>, A2: MyAir<EF>>(
    prover_state: &mut FSProver<EF, impl FSChallenger<EF>>,
    univariate_skips: usize,
    tables_1: &[&AirTable<EF, A1>],
    tables_2: &[&AirTable<EF, A2>],
    witnesses_1: &[AirWitness<'a, PF<EF>>],
    witnesses_2: &[AirWitness<'a, EF>],
) -> Vec<Vec<Evaluation<EF>>> {
    prove_many_air_3::<_, _, A2, _>(
        prover_state,
        univariate_skips,
        tables_1,
        &[],
...
fn prove_many_air_3<'a, EF: ExtensionField<PF<EF>>, A1: MyAir<EF>, A2: MyAir<EF>, A3: MyAir<EF>>(
    prover_state: &mut FSProver<EF, impl FSChallenger<EF>>,
    univariate_skips: usize,
    tables_1: &[&AirTable<EF, A1>],
    tables_2: &[&AirTable<EF, A2>],
...
fn open_unstructured_columns<'a, EF: ExtensionField<PF<EF>>>(
    ...
    let log_max_columns_per_group = witnesses_1
        .iter()
        .chain(witnesses_2.iter())
        .map(|w| w.log_max_columns_per_group())
        .chain(witnesses_3.iter().map(|w| w.log_max_columns_per_group()))
        .max()
        .unwrap();
    let columns_batching_scalars = prover_state.sample_vec(log_max_columns_per_group);
    ...
    all_evaluations_remaining_to_prove.try_into().unwrap()
}
...
fn open_structured_columns<'a, EF: ExtensionField<PF<EF>> + ExtensionField<IF>, IF: Field>(
    ...
    for witness in witnesses_1.iter().chain(witnesses_2) {
        for group in &witness.column_groups {
            ...
            log_n_rows.push(witness.log_n_rows());
    ...
    for ((batched_column, batched_column_mixed), &log_n_rows) in all_batched_columns
        .into_iter()
    ...
    let n_groups = witnesses_1
        .iter()
        .chain(witnesses_2.iter())
        .map(|w| w.column_groups.len())
        .chain(witnesses_3.iter().map(|w| w.column_groups.len()))
        .sum();
    ...
    for i in 0..n_groups {
        let point = MultilinearPoint(
            [
                from_end(&columns_batching_scalars, log2_ceil_usize(group_lengths[i])).to_vec(),
                inner_challenges.0[..log_n_rows[i]].to_vec(),
            ]
            .concat(),
        );
    ...
    let mut res = vec![];
    let mut counter = 0;
    for n_groups in witnesses_1
        .iter()
        .chain(witnesses_2)
        .map(|w| w.column_groups.len())
        .chain(witnesses_3.iter().map(|w| w.column_groups.len()))
    {
        res.push(evaluations_remaining_to_prove[counter..counter + n_groups].to_vec());
        counter += n_groups;
    }
    assert_eq!(counter, evaluations_remaining_to_prove.len());
    res
}
